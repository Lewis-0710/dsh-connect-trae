...
> 
> **与上游差异**：
> 1. **入口与 Provider 分组定制**：供应商名称统一定制为 `Trae` / `Trae Global`（含自动识图通道）。
> 2. **会员计划与倍率展示**：精准识别受限模型并展示「会员计划」标识；模型展示名内嵌积分倍率（如 `(0.77x)`），保持底层调用 ID 纯净。
> 3. **全模型 1M 上下文自由可选**：为所有模型提供 1M 上下文单选切换项，同时保留模型原生元数据与真实展示。
> 4. **本地 SQLite 深度解析 (`state.vscdb`)**：优先读取本地活动折扣与 IDE 最新模型；全面兼容解析上下文窗口字段，避免显示「最大上下文 未知」。
> 5. **全链路双重去重**：在本地缓存读取与多源合并时实施 ID + 展示名双重去重，彻底消除国际版 `Gemini-3.1-Pro-Preview`、`Gemini-3-Flash-Preview` 等重复模型。
> 6. **流式错误优雅传播**：上游报错时以 delta 文本块平稳向前端输出并正常结束流，避免连接突兀异常中断。
> 7. **配置持久化与智能默认**：持久化保留会员计划、倍率与上下文预算；「恢复默认」优先匹配 $\ge 1\text{M}$ 模型与最佳上下文配置。
> 
